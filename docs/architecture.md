...
Operações críticas usam `Prisma.$transaction`. A API retorna `{ data, meta }`. O Next.js não calcula regras financeiras e usa React Query como camada de servidor. Datas civis financeiras são persistidas como `date` e construídas em UTC; exibição usa `pt-BR`.
